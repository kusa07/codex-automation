...
try{
  ...
  $state.DispatchFenced=$true;$state.LegacyRegistered=$true;$state.TargetRegistered=$false;$state.Quiescent=$true;$state.TargetHostPrepared=$false;$state.PackageVerified=$false;$state.AclExact=$false;$actions.Clear();Invoke-Phase12BMigrationLifecycle -Identity $identity -InitialStage DISPATCH_FENCING -ReadState $readState -Mutate $mutate -Persist $persist|Out-Null;if($actions -contains 'FenceDispatch'){throw 'post-request DISPATCH_FENCING repeated fence'}
