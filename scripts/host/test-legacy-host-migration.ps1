...
try{
  ...
  $pageTwoMatch=Get-Phase12BLegacyRunnerIdentityMatch -LocalRunner $pageTwoLocal -CallerRepository 'example/paged' -CallerRepositoryId '999999999' -ActualRepositoryId '999999999' -ActualRepositoryFullName 'example/paged' -GitHubRunners $page101.Runners -ExpectedLabels @('self-hosted','Windows','X64','codex-automation')
  ...
  $otherMatch=Get-Phase12BLegacyRunnerIdentityMatch -LocalRunner $otherLocal -CallerRepository 'example/other-repo' -CallerRepositoryId '999999999' -ActualRepositoryId '999999999' -ActualRepositoryFullName 'example/other-repo' -GitHubRunners $otherGitHub -ExpectedLabels @('self-hosted','Windows','X64','codex-automation')
  ...
  $fullNameMismatch=Get-Phase12BLegacyRunnerIdentityMatch -LocalRunner $otherLocal -CallerRepository 'example/other-repo' -CallerRepositoryId '999999999' -ActualRepositoryId '999999999' -ActualRepositoryFullName 'example/recreated' -GitHubRunners $otherGitHub -ExpectedLabels @('self-hosted','Windows','X64','codex-automation');if(-not $fullNameMismatch.RepositoryMismatch){throw 'repository full_name mismatch accepted'}
  $missingFullName=Get-Phase12BLegacyRunnerIdentityMatch -LocalRunner $otherLocal -CallerRepository 'example/other-repo' -CallerRepositoryId '999999999' -ActualRepositoryId '999999999' -ActualRepositoryFullName '' -GitHubRunners $otherGitHub -ExpectedLabels @('self-hosted','Windows','X64','codex-automation');if(-not $missingFullName.RepositoryMismatch){throw 'missing repository full_name accepted'}
  $repoMismatch=Get-Phase12BLegacyRunnerIdentityMatch -LocalRunner $otherLocal -CallerRepository 'example/other-repo' -CallerRepositoryId '999999999' -ActualRepositoryId '888888888' -ActualRepositoryFullName 'example/other-repo' -GitHubRunners $otherGitHub -ExpectedLabels @('self-hosted','Windows','X64','codex-automation');if(-not $repoMismatch.RepositoryMismatch){throw 'repository ID mismatch accepted'}
  $urlMismatch=Copy-Object $otherLocal;$urlMismatch.gitHubUrl='https://github.com/example/wrong';$urlResult=Get-Phase12BLegacyRunnerIdentityMatch -LocalRunner $urlMismatch -CallerRepository 'example/other-repo' -CallerRepositoryId '999999999' -ActualRepositoryId '999999999' -ActualRepositoryFullName 'example/other-repo' -GitHubRunners $otherGitHub -ExpectedLabels @('self-hosted','Windows','X64','codex-automation');if(-not $urlResult.RepositoryMismatch){throw '.runner repository URL mismatch accepted'}
  ...
  # Dispatch ownership is durable: an already-disabled state is only valid at
  # DISPATCH_FENCING (a known post-request crash), never at LEGACY_VERIFIED.
  $state.DispatchFenced=$true;$state.DispatchStateKnown=$true;$state.LegacyRegistered=$true;$state.TargetRegistered=$false;$state.TargetHostPrepared=$false;$state.Quiescent=$false;$state.PackageVerified=$false;$state.AclExact=$false;$actions.Clear()
  Assert-Throws {Invoke-Phase12BMigrationLifecycle -Identity $identity -InitialStage LEGACY_VERIFIED -ReadState $readState -Mutate $mutate -Persist $persist} 'pre-disabled LEGACY_VERIFIED state was adopted'
  $state.DispatchFenced=$false;$state.LegacyRegistered=$true;$state.TargetRegistered=$false;$state.Quiescent=$true;$state.TargetHostPrepared=$false;$state.PackageVerified=$false;$state.AclExact=$false;$actions.Clear();Invoke-Phase12BMigrationLifecycle -Identity $identity -InitialStage DISPATCH_FENCING -ReadState $readState -Mutate $mutate -Persist $persist|Out-Null;if($actions -notcontains 'FenceDispatch'){throw 'active DISPATCH_FENCING did not fence'}
  $state.DispatchFenced=$true;$state.LegacyRegistered=$true;$state.TargetRegistered=$false;$state.Quiescent=$true;$state.TargetHostPrepared=$false;$state.PackageVerified=$false;$state.AclExact=$false;$actions.Clear();Assert-Throws {Invoke-Phase12BMigrationLifecycle -Identity $identity -InitialStage DISPATCH_FENCING -ReadState $readState -Mutate $mutate -Persist $persist} 'ambiguous disabled DISPATCH_FENCING was adopted';if($actions.Count -ne 0){throw 'ambiguous dispatch fencing performed a mutation'}
  # Quiescence is re-read immediately before unregister, including resume.
  $state.DispatchFenced=$true;$state.LegacyRegistered=$true;$state.TargetRegistered=$false;$state.TargetHostPrepared=$true;$state.PackageVerified=$true;$state.AclExact=$true;$state.Quiescent=$true;$state|Add-Member -NotePropertyName quiescence_behavior -NotePropertyValue 'BUSY' -Force;$actions.Clear();Assert-Throws {Invoke-Phase12BMigrationLifecycle -Identity $identity -InitialStage TARGET_HOST_PREPARED -ReadState $readState -Mutate $mutate -Persist $persist} 'busy target preparation was accepted';if($actions -contains 'UnregisterLegacy'){throw 'busy target preparation unregistered legacy runner'}
  $actions.Clear();Assert-Throws {Invoke-Phase12BMigrationLifecycle -Identity $identity -InitialStage LEGACY_RUNNER_UNREGISTERING -ReadState $readState -Mutate $mutate -Persist $persist} 'busy unregister resume was accepted';if($actions -contains 'UnregisterLegacy'){throw 'busy unregister resume mutated legacy runner'};$state.PSObject.Properties.Remove('quiescence_behavior')

  # Every durable stage must resume through the same production decision function.
  $allStages=@('LEGACY_VERIFIED','DISPATCH_FENCING','DISPATCH_FENCED','QUIESCENT','TARGET_HOST_PREPARED','LEGACY_RUNNER_UNREGISTERING','LEGACY_RUNNER_UNREGISTERED','TARGET_RUNNER_REGISTERING','TARGET_RUNNER_REGISTERED','SERVICE_INSTALLING','SERVICE_INSTALLED','SERVICE_RUNNING','ACTIVE_VERIFIED','DISPATCH_RESTORING','DISPATCH_RESTORED','MIGRATION_COMPLETE')
  foreach($start in $allStages){
    $state.DispatchFenced=$start -notin @('LEGACY_VERIFIED','DISPATCH_FENCING','DISPATCH_RESTORED','MIGRATION_COMPLETE');$state.Quiescent=$true;$state.TargetHostPrepared=$start -notin @('LEGACY_VERIFIED','DISPATCH_FENCING','DISPATCH_FENCED','QUIESCENT');$state.PackageVerified=$state.TargetHostPrepared;$state.AclExact=$state.TargetHostPrepared
    $state.LegacyRegistered=$start -in @('LEGACY_VERIFIED','DISPATCH_FENCING','DISPATCH_FENCED','QUIESCENT','TARGET_HOST_PREPARED','LEGACY_RUNNER_UNREGISTERING')
