...
$intentDirectory = Join-Path $h.runtime_root 'system-python'
$intentPath = Join-Path $intentDirectory 'python313-intent.json'
...
function Get-MachineEnvironmentHash([string]$Name) {
    $value = [string][Environment]::GetEnvironmentVariable($Name,'Machine')
    $bytes = [Text.Encoding]::UTF8.GetBytes($value)
    $sha = [Security.Cryptography.SHA256]::Create()
    try { [BitConverter]::ToString($sha.ComputeHash($bytes)).Replace('-','').ToLowerInvariant() }
    finally { $sha.Dispose() }
}
function Get-PythonIsolation($Runtime) {
    if (-not $Runtime.PathIsolated) { return 'CONFLICT' }
    if ($null -ne $script:intent -and
        ((Get-MachineEnvironmentHash 'Path') -cne [string]$script:intent.machine_path_sha256 -or
         (Get-MachineEnvironmentHash 'PATHEXT') -cne [string]$script:intent.machine_pathext_sha256)) { return 'CONFLICT' }
    'ISOLATED'
}
function Get-PythonDecision($State) {
    if ($State.HostState -ne 'EXISTING' -or -not $State.CallerExact -or -not $State.ServiceExact -or
        ...
        $State.RuntimeClassification -eq 'CONFLICT' -or $State.DispatchState -notin @('active','disabled_manually') -or
        $State.PathIsolation -ne 'ISOLATED') { return 'CONFLICT' }
    if ($State.ServiceState -ne 'Running' -or -not $State.RunnerOnlineIdle) { return 'CONFLICT' }
    if ($State.RuntimeClassification -eq 'EXACT') { return 'REFRESH_REQUIRED' }
    if ($State.RuntimeClassification -eq 'MISSING') { return 'INSTALLABLE' }
    'CONFLICT'
}
$script:intent = $null
...
function Get-ActualState {
    if($TestMode){
        ...
        $runtime=Get-Phase12BSystemPythonState -Observation $fixture.runtime_observation
        if ($runtime.Classification -eq 'CONFLICT') { throw 'System Python runtime classification is conflicted before resume mutation.' }
        ...
        return [pscustomobject]@{
            ...
            Quiescent=([bool]$fixture.quiescent -and [string]$fixture.quiescence_reason -ceq 'NO_CURRENT_EXECUTION');RuntimeClassification=$runtime.Classification;PathIsolation=(Get-PythonIsolation $runtime);PackageVerified=[bool]$fixture.package_verified;UnknownState=([bool]$fixture.unknown_state -or $runtime.Classification -eq 'CONFLICT' -or (Get-PythonIsolation $runtime) -ne 'ISOLATED')
        }
    }
    $runtime = Get-Phase12BSystemPythonState
    if ($runtime.Classification -eq 'CONFLICT') { throw 'System Python runtime classification is conflicted before resume mutation.' }
    ...
    if ($packageVerified) { $packageVerified = [string](Get-FileHash -LiteralPath $packagePath -Algorithm SHA256).Hash -ieq $policy.Sha256 -and (Test-Phase12BPythonSignature $packagePath) }
    [pscustomobject]@{
        ...
        Quiescent=($quiet.Result -eq 'PASS' -and $quiet.Reason -eq 'NO_CURRENT_EXECUTION'); RuntimeClassification=$runtime.Classification; PathIsolation=(Get-PythonIsolation $runtime)
        PackageVerified=$packageVerified; UnknownState=($runtime.Classification -eq 'CONFLICT' -or (Get-PythonIsolation $runtime) -ne 'ISOLATED')
    }
}
function Assert-IntentDirectory {
    ...
    $unexpected=@(Get-ChildItem -LiteralPath $intentDirectory -Force -ErrorAction Stop|Where-Object{$_.Name -cnotin @('python313-intent.json','packages')})
...
function Read-Intent {
    ...
    if ([string]$intent.machine_path_sha256 -notmatch '^[0-9a-f]{64}$' -or [string]$intent.machine_pathext_sha256 -notmatch '^[0-9a-f]{64}$' -or
        [string]$intent.schema -ne '1' -or [string]$intent.operation -cne 'SYSTEM_PYTHON313' -or [string]$intent.host_id -cne [string]$h.host_id -or
...
function Save-Intent([string]$Stage) {
    ...
    Assert-IntentDirectory
    $temp = Join-Path $intentDirectory ('.python313-intent.' + [guid]::NewGuid().ToString('N') + '.tmp')
    try {
...
$powershellIntent = Join-Path (Join-Path $h.runtime_root 'system-runtime') 'powershell7-intent.json'
if (Test-Path -LiteralPath $powershellIntent) {
    try { $other = Get-Content -LiteralPath $powershellIntent -Raw -ErrorAction Stop | ConvertFrom-Json -ErrorAction Stop } catch { throw 'PowerShell runtime intent is malformed; concurrent dependency apply is prohibited.' }
    if ([string]$other.operation -cne 'SYSTEM_POWERSHELL7' -or [string]$other.stage -cne 'COMPLETE') { throw 'PowerShell runtime apply is incomplete; concurrent dependency apply is prohibited.' }
}
$script:intent = Read-Intent
$initial = Get-ActualState
if ($null -eq $script:intent) {
    $decision = Get-PythonDecision $initial
    @('SYSTEM_RUNTIME_PLAN',"HOST_STATE=$hostState","CALLER=$($caller.Repository)","SERVICE=$($identity.ServiceName)","PYTHON313=$($initial.RuntimeClassification)","DISPATCH_INITIAL=$($initial.DispatchState)","DECISION=$decision",("RESULT={0}" -f $(if($Approve){'APPLY'}else{'PLAN'}))) -join "`n"
    ...
    Assert-IntentDirectory
    $script:intent = [pscustomobject]@{schema=1;operation='SYSTEM_PYTHON313';host_id=$h.host_id;repository_id=$caller.RepositoryId;repository_full_name=$caller.Repository;service_name=$identity.ServiceName;runner_id=$initial.RunnerId;initial_dispatch_state=$initial.DispatchState;version=$policy.Version;sha256=$policy.Sha256;machine_path_sha256=(Get-MachineEnvironmentHash 'Path');machine_pathext_sha256=(Get-MachineEnvironmentHash 'PATHEXT');stage='PLANNED';state_entered_at=[DateTimeOffset]::UtcNow.ToString('o')}
...
$mutate = {
    ...
    if($TestMode){
        ...
        switch($action){
            ...
            'InstallRuntime'{$fixture.runtime_observation=[pscustomobject]@{DirectoryPresent=$true;DirectoryIsContainer=$true;DirectoryNonReparse=$true;LeafPresent=$true;LeafNonReparse=$true;SignatureValid=$true;Version=$policy.Version;X64=$true;PathIsolated=$true}}
    ...
    switch ($action) {
        ...
        'VerifyPackage' { Get-Phase12BVerifiedSystemPythonPackage -CacheRoot $intentDirectory | Out-Null }
        'InstallRuntime' { Install-Phase12BSystemPython -CacheRoot $intentDirectory | Out-Null }
...
Invoke-Phase12BSystemRuntimeLifecycle -InitialStage ([string]$script:intent.stage) -InitialDispatchState ([string]$script:intent.initial_dispatch_state) -Read $read -Mutate $mutate -Save $save -RuntimeKind Python313 | Out-Null
'SYSTEM_PYTHON_APPLY=PASS'
